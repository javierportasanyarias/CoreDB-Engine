...
void disk_io::write_aux_val_buffer_v3(Values value, dataType tipo_dato, std::array<char, 128>& buffer, uint32_t& bytes_escritos_buffer, uint32_t& size_disponible_buffer, uint32_t& offset){
   ...
   */
   ...
   switch(tipo_dato){
      ...
      case dataType::STRING: {
         ...
         tmp_char_ptr = reinterpret_cast<char*>(&string_size);
         if(sizeof(uint32_t) + string_size > size_disponible_buffer){
            offset = sizeof(uint32_t) + string_size;
            return;
         };
         std::memcpy(
            buffer.data() + bytes_escritos_buffer, // 1. Destino (puntero al inicio + desplazamiento)
...
void disk_io::write_table_data_with_buffer_2(table* tabla, uint32_t n_rows){
   ...
   while(contador_fila < n_f_ram){
      ...
      while(contador_columnas < n_cols){
         // Recuperamos el valor concreto a recuperar:
         if(!offset){
            valor_tmp = fila_a_escribir.at(columnas_nombre[contador_columnas]);
         };
   ...
   // Por ultimo escribimos lo sobrante:
   out.write(buffer.data(), bytes_escritos_buffer);
   out.flush();
   out.close();
};
...
void disk_io::write_table_data_with_buffer_3(table* tabla, uint32_t n_rows){
   ...
   while(contador_fila < n_f_ram){
      ...
      while(contador_columnas < n_cols){
         ...
         disk_io::write_aux_val_buffer_v3(valor_tmp, tipos_datos[contador_columnas], buffer, bytes_escritos_buffer, size_disponible_buffer, offset);
         ...
         if (table_ptr != nullptr){
            ...
            if(table_ptr->data_ptr){
               if(!table_ptr->data_ptr->columns.empty()){
                  ...
                  //disk_io::write_table_data_with_buffer_2(table_ptr, n_rows);
                  disk_io::write_table_data_with_buffer_3(table_ptr, n_rows);
