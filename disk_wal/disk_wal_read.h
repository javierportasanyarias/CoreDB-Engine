...
char* recover_meta_wal_table_buffer(std::ifstream& in,
                                    const std::string& table_name_str_input);

std::string recover_meta_wal_table_name(std::ifstream& in);

void aux_read_single_table_wal_metadata(
...
class walDataReader {
 public:
  ...
  bool is_eof_rows() {
    return this->col_counter >= this->num_cols &&
           this->row_counter + 1 >= this->num_rows;
  };
...
class walDataParser {
 public:
  ...
  walDataParser() : eof_file(false), wal_data_type(0), state(0) {
