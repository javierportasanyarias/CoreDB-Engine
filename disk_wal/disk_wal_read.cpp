...
char* disk_wal_read::recover_meta_wal_table_buffer(
    std::ifstream& in, const std::string& table_name_str_input) {
  /*
  Función que devuelve un buffer con los metadatosdatos de ua tabla concreta,
  dado su nombre.
  */
  ...
  if (fs::exists("metadata/" + table_name_str_input + "_meta.bin"))
    return buffer;
  Logger::log(LogLevel::DEBUG,
              "Metadata file does not exist, thus we carry on");
  ...
  // 1º) Leemos el tamaño del buffer de memoria de los metadatos de la tabla
  // concreta:
...
std::string disk_wal_read::recover_meta_wal_table_name(std::ifstream& in) {
  /*
  Función que devuelve el nombre de la tabla de la que actualmente se está
  leyendo los datos/metadatos
  */
...
void disk_wal_read::aux_read_single_table_wal_metadata(
    std::ifstream& in, const std::string& table_name_str_input) {
  /*
  Función auxliar que lee los metadatos de una sola tabla desde el WAL de
  ...
  */
  ...
  char* buffer = nullptr;
  buffer =
      disk_wal_read::recover_meta_wal_table_buffer(in, table_name_str_input);
  ...
  if (it != global_table_dict.end()) {
    Logger::log(LogLevel::DEBUG,
                "Metadata has already been loades on to the table's metadata "
                "section. Therefore, we skip the next step");
    return;
  };
  ...
  Logger::log(LogLevel::DEBUG, " does not exist. Proceeding to create it", true,
              false);
  ...
  Logger::log(LogLevel::DEBUG,
              "Creating a new metadata pointer for the newly spawned table");
  ...
  /////////////////////////////////////////////
  // Ya recuperado el buffer de datos, debemos leerlo y rellenar los metadatos
  // de la tabla en RAM
  ...
  Logger::log(LogLevel::DEBUG,
              "Entering the column loop (for registering it within the "
              "metadata segment)");
...
bool disk_wal_read::is_eof_read(std::ifstream& in) {
  ...
  if (current_pos == std::streampos(-1)) {
    Logger::log(LogLevel::DEBUG,
                "[EOF] Pointer value is -1. End of file or reading failure.");
    return true;
  }
  ...
  // 5º Traducimos las posiciones a números enteros normales (bytes) para operar
  // con ellos
  ...
  Logger::log(LogLevel::DEBUG,
              " -> Current reading pointer is in byte: ", false, true);
  ...
  // Si queda 1 byte (por ejemplo, un '\n' o un 0x00 residual del formateo),
  // también es el final.
  if (remaining_bytes <= 1) {
    Logger::log(
        LogLevel::DEBUG,
        "[EOF] DETECTED: 1 or 0 bytes reamain. Stoping the parsing procedure.");
    return true;
  }

  Logger::log(LogLevel::DEBUG,
              "[EOF] NOT DETECTED: We carry on with the reading.");
  return false;
};
  ...
  switch (this->data_types[this->col_counter]) {
    case dataType::INT: {
      ...
      if (var_size <= this->buffer_bytes_available) {
        ...
        this->value = int_val;
        Logger::log(LogLevel::DEBUG,
                    "Buffer data copied to the variant value successfully");

        (*this->columns)[this->col_names[this->col_counter]].push_back(
            this->value);
    ...
    case dataType::STRING: {
      ...
      if (!this->len_read) {
        if (var_size <= this->buffer_bytes_available) {
          ...
          std::memcpy(&this->var_len_bytes, this->data_buffer_pointer,
                      var_size);
          this->bytes_read += var_size;
          this->buffer_bytes_read += var_size;
          this->buffer_bytes_available -= var_size;
          ...
          Logger::log(LogLevel::DEBUG,
                      "Creating a temporary string for storing current string "
                      "data from disk");
          this->value = std::string();
          Logger::log(
              LogLevel::DEBUG,
              "String created and asociated with variant value successfully");
          std::string& str_tmp = std::get<std::string>(this->value);
          Logger::log(LogLevel::DEBUG,
                      "Access to variant value string successful");
          str_tmp.reserve(this->var_len_bytes);
          Logger::log(LogLevel::DEBUG, "String content reservation successful");
          Logger::log(LogLevel::DEBUG, "Number of bytes reserved:: ", false,
                      true);
      ...
      if (this->len_read && this->var_len_bytes > 0) {
        ...
        if (this->var_len_bytes > 0 && this->buffer_bytes_available == 0) {
          Logger::log(
              LogLevel::DEBUG,
              "String fragmented into WAL blocks. Forcing offset condition.");
          this->offset = true;
      ...
      if (this->len_read && this->var_len_bytes == 0) {
        Logger::log(
            LogLevel::DEBUG,
            "String has been read whole or is empty. Saving into the table.");

        (*this->columns)[this->col_names[this->col_counter]].push_back(
            this->value);

        // Reseteo de flags de control para permitir avanzar a la siguiente
        // columna
        this->col_counter += 1;
        this->var_len_bytes = 0;
        this->var_len_bytes_read = 0;
    ...
    case dataType::UNKNOWN: {
      ...
      if (!this->len_read) {
        if (var_size <= this->buffer_bytes_available) {
          ...
          Logger::log(LogLevel::DEBUG,
                      "Creating a temporary character array for storing "
                      "current string data from disk");
          this->value = std::vector<char>();
          Logger::log(
              LogLevel::DEBUG,
              "Array created and asociated with variant value successfully");
          std::vector<char>& vec_tmp = std::get<std::vector<char>>(this->value);
          Logger::log(LogLevel::DEBUG,
                      "Access to variant value array successful");
      ...
      if (this->len_read && this->var_len_bytes > 0) {
        ...
        if (bytes_to_read_tmp > 0) {
          Logger::log(LogLevel::DEBUG,
                      "Recovering variant value's character array:");
          std::vector<char>& vec_tmp = std::get<std::vector<char>>(this->value);
          vec_tmp.insert(vec_tmp.end(), this->data_buffer_pointer,
                         this->data_buffer_pointer + bytes_to_read_tmp);
        ...
        if (this->var_len_bytes > 0 && this->buffer_bytes_available == 0) {
          Logger::log(
              LogLevel::DEBUG,
              "Array fragmented into WAL blocks. Forcing offset condition.");
          this->offset = true;
          break;  // Salimos del case para ir al ciclo de recarga de buffer
        };
      };
      ...
      if (this->len_read && this->var_len_bytes == 0) {
        Logger::log(
            LogLevel::DEBUG,
            "Array has been read whole or is empty. Saving into the table.");
...
void disk_wal_read::walDataReader::control_unit() {
  switch (this->state) {
    case 0: {
      ...
      this->in.read(reinterpret_cast<char*>(&this->buffer_tmp_size),
                    sizeof(uint32_t));
    ...
    case 255: {
      ...
      Logger::log(LogLevel::DEBUG,
                  "Metadata's 'n_rows_ram' has been filled with the number of "
                  "rows successfully: ",
                  false, true);
...
void disk_wal_read::walDataParser::control_unit() {
  switch (this->state) {
    ...
    case 2: {
      ...
      this->in.read(reinterpret_cast<char*>(&this->wal_data_type),
                    sizeof(uint8_t));
      this->state = 3;
      break;
    };
    ...
    case 4: {
      ...
      // Metadata read:
      disk_wal_read::aux_read_single_table_wal_metadata(this->in,
                                                        this->table_name);
