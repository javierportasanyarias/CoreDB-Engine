...
dataType transform_str_to_datatype(const std::string& input){
   /*
   Function that given a string that signals data type, returns such datatype flag
   */
   if (input == "INT"){
      return dataType::INT;
   } else if(input == "FLOAT"){
...
//////////////////////////////////////////////////////////////////////////
   ...
   for(int j = 0; j<row_num; j++){
      for(int i = 0; i<len_n_cols; i++){
         ...
         // Variant value ready, now we move it to the table's data section (current execution/seesion data section):
...
// Auxiliary function that prints the table's values (ALL COLUMNS SELECTION CASE):
void aux_table_values_print(const std::vector<std::string>& col_list, std::string table_name) {
   ...
   std::map<std::string, Values> row_map;
	...
	while(!it.is_eof()){
      ...
      if (row_map.empty()) {
         // Guard condtition against empty rows before EOF flagging has been swiched on
         continue;
      };
...
// Auxiliary function that prints the table's values (ARBITRARY COLUMNS SELECTION CASE):
