...
// String overload
void Logger::log(LogLevel msgLevel, const std::string& msg, bool flush_bool,
                 bool flag) {
...
*/
void Logger::log(LogLevel msgLevel, const char* msg, bool flush_bool,
                 bool flag) {
  log(msgLevel, std::string(msg), flush_bool, flag);
};

// Int overload
void Logger::log(LogLevel msgLevel, const int msg, bool flush_bool, bool flag) {
  if (msgLevel < level) return;
...
void Logger::log(LogLevel msgLevel, const std::filesystem::path msg,
                 bool flush_bool, bool flag) {
  if (msgLevel < level) return;
...
void Logger::log_buffer(LogLevel msgLevel, const char* buffer,
                        uint32_t buffer_len, bool flush_bool, bool flag) {
  if (msgLevel < level) return;
  ...
  // We proceed to print the buffer both as raw hexadecimal and converted to
  // alphanumeric characters Main purpose of this logger is to help debug string
  ...
  for (uint32_t i = 0; i < buffer_len; ++i) {
    ...
    // We only take into account ASCII alphanumeric characters, leaving out
    // control
    if (byte >= 32 && byte <= 126) {
      ss_txt << (char)byte
             << "  ";  // Añadimos espacios para alinear visualmente con el hex
    } else {
...
void Logger::flush(LogLevel msgLevel, bool endl) {
  if (msgLevel < level) return;
