...
void disk_out::fix_vect_vals(std::vector<Values>::iterator start_iterator,
...
void disk_out::obtain_writing_indexes(auto& idx_start, auto& idx_end,
                                      std::vector<Values>*& vec_var_ptr,
                                      int written_rows_input,
                                      int& rows_to_write_input) {
  idx_start = vec_var_ptr->begin() + written_rows_input;
  idx_end = vec_var_ptr->begin() + written_rows_input + rows_to_write_input;
};

int disk_out::obtain_available_size(const std::filesystem::path path_var_input,
                                    const std::string& last_partition_str_input,
                                    int size_partition_input) {
...
void disk_out::write_partition_int(auto& idx_start, auto& idx_end,
                                   std::filesystem::path write_path_input) {
  std::ofstream out;
...
void disk_out::write_partition_float(auto& idx_start, auto& idx_end,
                                     std::filesystem::path write_path_input) {
  std::ofstream out;
...
void disk_out::write_partition_bool(auto& idx_start, auto& idx_end,
                                    std::filesystem::path write_path_input) {
  std::ofstream out;
...
struct byteStreamer {
  ...
  */
  ...
  bool is_data_left() { return start_iterator < end_iterator; };

  // Function that will return the buffer full:
  uint32_t extract_bytes(char* buffer, uint32_t* buffer_sizes,
                         uint32_t limit_input) {
    written_rows = 0;
    ...
    if (this->is_unknown_type) {
      while (start_iterator < end_iterator && bytes_copied < limit_input) {
        ...
        if (string_remaining_bytes <= buffer_free_space) {
          ...
          std::memcpy(buffer + bytes_copied, str_begin + char_idx,
                      string_remaining_bytes);
...
void disk_out::write_partition_string(
    std::vector<Values>::iterator start_iterator,
    std::vector<Values>::iterator end_iterator,
    std::filesystem::path write_path_input) {
  // The 'int available_space_to_write' parameter is unnecessary as input.
  // 'start_iterator' and 'end_iterator' already define the range of rows to
  // write in a partition.
  ...
  try {
    ...
    while (byte_streamer.is_data_left()) {
      bytes_copied =
          byte_streamer.extract_bytes(arr_vals, arr_sizes, size_buffer_bytes);

      out1.write(arr_vals, bytes_copied);
      out1.flush();
      char* buffer_sizes_chars = reinterpret_cast<char*>(arr_sizes);
      out2.write(buffer_sizes_chars,
                 byte_streamer.written_rows * sizeof(uint32_t));
      out2.flush();
    };
  } catch (...) {
    delete[] arr_vals;
...
void disk_out::write_partition_unknown(
    std::vector<Values>::iterator start_iterator,
    std::vector<Values>::iterator end_iterator,
    std::filesystem::path write_path_input) {
  // The 'int available_space_to_write' parameter is unnecessary as input.
...
void disk_out::writing_loop_int(std::vector<Values>*& vec_var_ptr,
                                int total_bytes_to_write_input,
                                int partition_entities,
                                std::filesystem::path path_var_input,
                                std::string last_partition_str_input,
                                int last_partition_int_input) {
  ...
  while (total_bytes_to_write_input > bytes_written) {
    ...
    */
    available_space_to_write = disk_out::obtain_available_size(
        path_var_input, last_partition_str_input + ".dat", size_partition);
    Logger::log(LogLevel::DEBUG,
                "Available space within the partition file is: ", false, true);
    ...
    if (rows_to_write > 0) {
      ...
      disk_out::obtain_writing_indexes(start_iterator, end_iterator,
                                       vec_var_ptr, written_rows,
                                       rows_to_write);
    ...
    Logger::log(LogLevel::DEBUG,
                "'last_partition_str_input' value before: ", false, true);
    ...
    Logger::log(LogLevel::DEBUG,
                "'last_partition_str_input' value after: ", false, true);
    Logger::log(LogLevel::DEBUG, last_partition_str_input, true, false);
  };
};

void disk_out::writing_loop_float(std::vector<Values>*& vec_var_ptr,
                                  int total_bytes_to_write_input,
                                  int partition_entities,
                                  std::filesystem::path path_var_input,
                                  std::string last_partition_str_input,
  ...
  while (total_bytes_to_write_input > bytes_written) {
    ...
    */
    ...
    rows_left_to_write = total_rows_to_write - written_rows;
    bytes_left_to_write = total_bytes_to_write_input - bytes_written;
    rows_that_fit_in_current_partition =
        available_space_to_write / sizeof(float);
    if (rows_left_to_write > rows_that_fit_in_current_partition) {
      rows_to_write = rows_that_fit_in_current_partition;
      bytes_to_write = available_space_to_write;
    } else {
...
void disk_out::writing_loop_bool(std::vector<Values>*& vec_var_ptr,
                                 int total_bytes_to_write_input,
                                 int partition_entities,
                                 std::filesystem::path path_var_input,
                                 std::string last_partition_str_input,
  ...
  while (total_bytes_to_write_input > bytes_written) {
    ...
    */
    ...
    bytes_left_to_write = total_bytes_to_write_input - bytes_written;
    rows_that_fit_in_current_partition =
        available_space_to_write / sizeof(uint8_t);
    if (rows_left_to_write > rows_that_fit_in_current_partition) {
      rows_to_write = rows_that_fit_in_current_partition;
      bytes_to_write = available_space_to_write;
    } else {
...
void disk_out::writing_loop_tring(std::vector<Values>*& vec_var_ptr,
                                  int total_bytes_to_write_input,
                                  int partition_entities,
                                  std::filesystem::path path_var_input,
                                  std::string last_partition_str_input,
  ...
  while (total_bytes_to_write_input > bytes_written) {
    ...
    */
    available_space_to_write = disk_out::obtain_available_size(
        path_var_input, last_partition_str_input + ".idx", size_partition);
    Logger::log(LogLevel::DEBUG,
                "Available space within the partition file is: ", false, true);
    Logger::log(LogLevel::DEBUG, available_space_to_write, true, false);
    // 2) Calculating rows to write:
    ...
    bytes_left_to_write = total_bytes_to_write_input - bytes_written;
    rows_that_fit_in_current_partition =
        available_space_to_write / sizeof(uint32_t);
    if (rows_left_to_write > rows_that_fit_in_current_partition) {
      rows_to_write = rows_that_fit_in_current_partition;
      bytes_to_write = available_space_to_write;
    } else {
    ...
    if (rows_to_write > 0) {
      ...
      std::filesystem::path write_path = path_var_input;
      write_path += last_partition_str_input;
      disk_out::write_partition_string(start_iterator, end_iterator,
                                       write_path);
      // 4) Once it is done, we update the writing counters:
      bytes_written += bytes_to_write;
      written_rows += rows_to_write;
...
void disk_out::writing_loop_unknown(std::vector<Values>*& vec_var_ptr,
                                    int total_bytes_to_write_input,
                                    int partition_entities,
                                    std::filesystem::path path_var_input,
                                    std::string last_partition_str_input,
  ...
  while (total_bytes_to_write_input > bytes_written) {
    ...
    */
    ...
    if (rows_to_write > 0) {
      ...
      std::filesystem::path write_path = path_var_input;
      write_path += last_partition_str_input;
      disk_out::write_partition_unknown(start_iterator, end_iterator,
                                        write_path);
      // 4) Once it is done, we update the writing counters:
      bytes_written += bytes_to_write;
      written_rows += rows_to_write;
...
void disk_out::write_table_data(table* table_ptr_input) {
  ...
  std::vector<dataType> column_types =
      table_ptr_input->metadata_ptr->column_types;
  // Retrieving column names:
  std::vector<std::string> column_names =
      table_ptr_input->metadata_ptr->column_names;
  // Retrieving data's vector of vectors:
  std::map<std::string, std::vector<Values>>& vec_de_vec_vals =
      table_ptr_input->data_ptr->columns;
  ...
  partition_names_str = disk_aux::obtain_files_in_path(
      table_path / column_names[0], column_types[0]);
  ...
  if (!partition_names_str.empty()) {
    ...
    if (last_partition != "") {
      ...
      last_partition_str =
          part_sort::process_partition_number_as_string(last_partition);
      Logger::log(LogLevel::DEBUG, "'last_partition_str' value is: ", false,
                  true);
  ...
  for (int i = 0; i < num_cols; i++) {
    ...
    std::filesystem::path column_path =
        std::filesystem::path("data") / table_name_str / column_name;
    ...
    switch (data_type) {
      case dataType::INT: {
        ...
        disk_out::writing_loop_int(vec_var_ptr, total_bytes_to_write,
                                   partition_entities, path_var,
                                   last_partition_str, last_partition_int);
        break;
      };
      case dataType::FLOAT: {
        total_bytes_to_write = vec_var_ptr->size() * 4;  // Total bytes to write

        disk_out::writing_loop_float(vec_var_ptr, total_bytes_to_write,
                                     partition_entities, path_var,
                                     last_partition_str, last_partition_int);
        break;
      };
      case dataType::BOOL: {
        ...
        disk_out::writing_loop_bool(vec_var_ptr, total_bytes_to_write,
                                    partition_entities, path_var,
                                    last_partition_str, last_partition_int);
        break;
      };
      case dataType::STRING: {
        total_bytes_to_write = vec_var_ptr->size() * 4;  // Total bytes to write
        disk_out::writing_loop_tring(vec_var_ptr, total_bytes_to_write,
                                     partition_entities, path_var,
                                     last_partition_str, last_partition_int);
        break;
      };
      case dataType::UNKNOWN: {
        total_bytes_to_write = vec_var_ptr->size() * 4;  // Total bytes to write
        disk_out::writing_loop_unknown(vec_var_ptr, total_bytes_to_write,
                                       partition_entities, path_var,
                                       last_partition_str, last_partition_int);
        break;
      };
