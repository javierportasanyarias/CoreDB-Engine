...
namespace execPlan {
    ...
    class queueNode1 {
        public:
            ...
            queueNode1(): nxt_node_queue(nullptr), prv_node_queue(nullptr){};
    ...
    public:
        ...
        Queue() : first_ptr(nullptr), last_ptr(nullptr) {}
...
recursive_tree_print(std::get<NodeType1*>(current->nodePtr));
	...
	void execute_queue_tasks(){
	   int index = 0;
           queueNode1* c_q_n = new queueNode1;
	   c_q_n = first_ptr;
	   std::cout << "Nodo " << index << ": ";
	   while(c_q_n){
	      ...
	      } else if (std::holds_alternative<NodeType2*>(c_q_n->nodePtr)) {                                                                              std::cout << "Nos encontramos al ejecutar un: NodeType2";                                         } else if (std::holds_alternative<NodeType3*>(c_q_n->nodePtr)) {                                                                              std::cout << "NodeType3: Ejecutamos insercion de datos"<<std::endl;
	   ...
	   if (index == 0){                                                         std::cout << "(La cola está vacía)\n";
	   };
	};
    };
