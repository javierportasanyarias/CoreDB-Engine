...
uint32_t disk_aux::obtain_file_size(const std::filesystem::path& path) {
   ...
   */
   ...
   if (!archivo.is_open()) {
      Logger::log(LogLevel::DEBUG, "ERROR while opening the file for obtaining the file size");
...
void disk_aux::write_aux_val_buffer_with_size_check(const Values& value, dataType data_type, char*& buffer_pointer, uint32_t& offset, uint32_t& bytes_written, uint32_t& bytes_remain, char*& ptr_str_ini, uint32_t& current_col){
   /*
      ...
      case dataType::STRING: {
         ...
         if(!ptr_str_ini){
            ...
            if(uint32_size <= bytes_remain){
               ...
               Logger::log(LogLevel::DEBUG, "[WRITE_STRING] [Phase 1] Size written successfully.");
               Logger::log(LogLevel::DEBUG, "[WRITE_STRING] [Phase 1] New ptr_str_ini assigned to: ", false, true); Logger::log(LogLevel::DEBUG, reinterpret_cast<uintptr_t>(ptr_str_ini), true, false);
               
               // If there are no bytes remaining, we do not let it continue to phase 2:
               if (bytes_remain == 0) {
                  Logger::log(LogLevel::DEBUG, "[WRITE_STRING] [Phase 1] Full buffer (bytes_remain == 0). Exiting.");
                  return;
               };
            }else{
               Logger::log(LogLevel::DEBUG, "[WRITE_STRING] [Phase 1] ERROR: Content does not fit inside the buffer. Setting new value for the offset.");
               offset = uint32_size;
               return;
            }; 
         };
         ...
         if (ptr_str_ini && bytes_remain > 0) {
            uint32_t bytes_remaining = (string_val.data() + string_size) - ptr_str_ini;
            
            Logger::log(LogLevel::DEBUG, "[WRITE_STRING] [Phase 2] ptr_str_ini valid. Calculating remaining bytes.");
            Logger::log(LogLevel::DEBUG, "[WRITE_STRING] [Phase 2] string_val.data() + string_size = ", false, true); Logger::log(LogLevel::DEBUG, reinterpret_cast<uintptr_t>(string_val.data() + string_size), true, false);
            Logger::log(LogLevel::DEBUG, "[WRITE_STRING] [Phase 2] Current ptr_str_ini = ", false, true); Logger::log(LogLevel::DEBUG, reinterpret_cast<uintptr_t>(ptr_str_ini), true, false);
            ...
            Logger::log(LogLevel::DEBUG, "[WRITE_STRING] [Phase 2] bytes_to_write: ", false, true); Logger::log(LogLevel::DEBUG, bytes_to_write, true, false);
         ...
         Logger::log(LogLevel::DEBUG, "[WRITE_STRING] [Phase 3] Final limit (data+size): ", false, true); Logger::log(LogLevel::DEBUG, reinterpret_cast<uintptr_t>(string_val.data() + string_size), true, false);
         
         if(ptr_str_ini >= string_val.data() + string_size){
            // We have finished reading the whole string:
            Logger::log(LogLevel::DEBUG, "^^^^^^^^ ptr_str_ini is now nullptr ^^^^^^^^");
            current_col += 1;
            ptr_str_ini = nullptr;
         }else{
            Logger::log(LogLevel::DEBUG, "^^^^^^^^ ptr_str_ini will NOT be nullptr ^^^^^^^^");
         };
         return;
      };
...
void disk_aux::fill_vector_int(uint32_t elements_num, char* vec_in, std::vector<Values>& vec_out){
...
void disk_aux::fill_vector_float(uint32_t elements_num, char* vec_in, std::vector<Values>& vec_out){
...
void disk_aux::fill_vector_bool(uint32_t elements_num, char* vec_in, std::vector<Values>& vec_out){
...
void disk_aux::scan_tables_recursive(const std::filesystem::path& path, std::vector<std::filesystem::path>& arr_tables){
   /*
   ...
   */
   ...
   for (const auto& entry : std::filesystem::directory_iterator(path)){
      ...
      } else if(std::filesystem::is_regular_file(entry) && entry.path().extension().string() == ".bin" && entry.path().filename().string().find("_meta") != std::string::npos){
