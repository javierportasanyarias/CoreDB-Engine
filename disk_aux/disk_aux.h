...
namespace disk_aux{
    ...
    std::vector<std::string> obtain_files_in_path(const std::filesystem::path& ruta, dataType tipo_dato);
    ...
    void fill_vector_int(uint32_t elements_num, char* vec_in, std::vector<Values>& vec_out);

    void fill_vector_float(uint32_t elements_num, char* vec_in, std::vector<Values>& vec_out);

    void fill_vector_bool(uint32_t elements_num, char* vec_in, std::vector<Values>& vec_out);
    ...
    void scan_tables_recursive(const std::filesystem::path& ruta, std::vector<std::filesystem::path>& arr_tablas);
