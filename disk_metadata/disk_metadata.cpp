...
void disk_metadata::read_table_metadata(std::filesystem::path path_table_input,
                                        std::string table_name_str) {
  table_metadata*& metadata_pointer =
      global_table_dict.at(table_name_str)->metadata_ptr;
  ...
  try {
    ...
    // Here we first delete the character buffer in the heap and then throw the
    // error:
    delete[] buffer_meta;
    throw;
  };
...
uint32_t disk_metadata::calculate_metadata_byte_size(
...
uint32_t disk_metadata::calculate_metadata_byte_size_wal(
    table_metadata* metadata_ptr) {
  /*
  Function designed to calculate the total byte size
  of the data stored within the table's metadata
...
uint32_t disk_metadata::write_table_metadata(table* table_ptr_input) {
  ...
  */
  ...
  // We do not make metadata pointer an alias as we do not need to modify any
  // values
  ...
  uint32_t meta_byte_size =
      disk_metadata::calculate_metadata_byte_size(metadata_pointer);
  if (meta_byte_size == 0) {
    return 0;
  };
  ...
  */
  ...
  try {
    ...
    std::map<std::string, std::vector<Values>> columnas =
