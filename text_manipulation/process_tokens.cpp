...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Auxiliar class (tableCatalog) for keeping track of the tables created o
// already present:
...
void process_list_to_define_schema(execPlan::Queue* excec_queue,
  ...
  'NodeType1' node, and adding it to the task queue.
  */

  Logger::log(LogLevel::DEBUG,
              "Inside the 'process_list_to_define_schema' function");
  ...
  // Now we fill all child nodes corresponding to the name and type of the
  // table's columns:
...
void insert_row_values_in_node(NodeType3*& node_ptr,
  ...
  Logger::log(LogLevel::DEBUG,
              "Vector insertion within the vector of vectors:");
  ...
  Logger::log(
      LogLevel::DEBUG,
      "Node for value insertion has successfully been creadted. Value: ", false,
      true);
  Logger::log(LogLevel::DEBUG, c_l_n->val, true, false);
};
...
void process_list_to_insert_values(execPlan::Queue* excec_queue,
  ...
  if (c_l_n->val == "VALUES") {
    ...
    Logger::log(LogLevel::DEBUG,
                "Node value that originated the error: ", false, true);
    Logger::log(LogLevel::DEBUG, c_l_n->val, true, false);
...
void aux_col_query(QueryNode*& query_node, textUtils::NodeList1*& c_l_n) {
  ...
  c_l_n = c_l_n->nxt_node;  // We skip to the coma
  // While loop based on the fact that the last element won't hace a coma
  // following it:
  while (c_l_n->val == "," && c_l_n->val != "EOS") {
    c_l_n = c_l_n->nxt_node;  // We skip to the value
    ItemNode node_item;
...
void process_list_for_selection(execPlan::Queue*& excec_queue,
  ...
  if (c_l_n->val == "FROM") {
    ...
    Logger::log(
        LogLevel::DEBUG,
        "FROM deteted. Checking if table is present in the global dictionary:");
    if (global_table_dict.find(c_l_n->val) == global_table_dict.end()) {
      if (!tableCatalog::ckeck_table(c_l_n->val)) {
        Logger::log(LogLevel::DEBUG,
                    "NO entry with that name was found within the global table "
                    "registry");
    ...
    Logger::log(LogLevel::DEBUG,
                "Table name does exist within the global dictionary");
...
void add_drop_table_node_to_queue(execPlan::Queue*& excec_queue,
                                  textUtils::NodeList1*& c_l_n) {
  /*
  Function destined to create a 'DropTableNode' node. This will be used for
  table erasure later on during instructions execution.
  */
  ...
  // In contrast with table creation, table deletion CAN look it up in the
  // global table dictionary:
  std::unordered_map<std::string, table*>::iterator item_pair =
      global_table_dict.find(table_name);
  if (item_pair == global_table_dict.end()) {
    // If the table is yet not created, we search it among the defined yet not
    // created tables:
...
execPlan::Queue* process_token_list(textUtils::simpleLinkedList*& list_ptr) {
  /*
  Main function for constructing a task queue containing all the execution
  trees. This queue then will be executed in an orderly manner, running each of
  the node's execution trees within.
  */
